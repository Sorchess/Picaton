      ...
      <div className="search-page__hero">
        ...
        {myCompanies.length > 0 && (
          <div className="search-page__company-filter">
            <button
              className={`search-page__company-filter-toggle ${
                isCompanyFilterOpen ? "active" : ""
              } ${selectedCompanyIds.size > 0 ? "has-selection" : ""}`}
            ...
            >
              ...
              Поиск в компаниях
              {selectedCompanyIds.size > 0 && (
                <span className="search-page__company-filter-badge">
                  {selectedCompanyIds.size}
                </span>
              )}
              <svg
                className={`search-page__company-filter-arrow ${
                  isCompanyFilterOpen ? "open" : ""
                }`}
                width="16"
                height="16"
                viewBox="0 0 24 24"
            ...
            {isCompanyFilterOpen && (
              <div className="search-page__company-list">
                {selectedCompanyIds.size > 0 && (
                  <button
                    className="search-page__company-clear"
                ...
                {myCompanies.map((item) => (
                  <label
                    key={item.company.id}
                    className={`search-page__company-item ${
                      selectedCompanyIds.has(item.company.id) ? "selected" : ""
                    }`}
                  >
                    <input
                      type="checkbox"
                      checked={selectedCompanyIds.has(item.company.id)}
                      onChange={() => toggleCompanySelection(item.company.id)}
                    />
                    <span className="search-page__company-logo">
                    ...
                    <span className="search-page__company-name">
                      {item.company.name}
                    </span>
                  </label>
                ))}

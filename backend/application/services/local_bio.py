"""Local LLM-based bio generator with streaming support."""
...
class LocalBioGenerator(AIBioGeneratorInterface):
    """
    Bio generator using local T-lite model via llama.cpp.

    Supports both standard and streaming generation.
    Falls back to simple template if local LLM fails.
    """
    ...
    async def generate_bio(self, user: User) -> GeneratedBio:
        """
        Generate professional bio using local LLM.
    ...
    async def stream_bio(
        ...
        """
        Stream bio generation for typewriter effect.
        ...
        if not parts:
            # Yield default bio for streaming
            default = self._default_bio(name)
            yield default
            return

        combined_text = " ".join(parts)
        user_prompt = (
            f"Имя: {name}\nИнформация: {combined_text}"
            if name
            else f"Информация: {combined_text}"
        )
        ...
        try:
            async for chunk in self._client.stream_complete(
                ...
                # Clean chunk from common prefixes
                cleaned = self._clean_chunk(chunk)
                if cleaned:
                    yield cleaned

        except LocalLLMError as e:
            logger.warning(f"Local LLM streaming error: {e.message}")
            # Fallback to default bio
            yield self._fallback_bio(combined_text, name)
    ...
    async def _generate(self, text: str, name: str) -> GeneratedBio:
        ...
        try:
            ...
            logger.warning(f"Local LLM error in generate_bio: {e.message}")
            return GeneratedBio(
    ...
    def _clean_chunk(self, chunk: str) -> str:
        """Clean streaming chunk."""
        # Remove common prefix patterns if they appear at start
        prefixes = ["Вот ", "Презентация:", "Описание:"]
